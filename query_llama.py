...
#     model = Llama2Lora(args.model_path)
    ...
    with tqdm(total=len(dataset)) as pbar:
        for ex in dataset:
            prediction = model.generate(texts=[ex["instruction"] + '\n\n' + ex["text"]], batch_size=args.generate_batch_size)
            predictions.append(prediction[0])  # Since generate returns a list of predictions, take the first one
